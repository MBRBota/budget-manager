...
export const registerUser = async (userCredentials) => {
  const { data } = await authApi.post(APP_CONSTANTS.API_ENDPOINTS.AUTH_ENDPOINTS.REGISTER_ENDPOINT, {
...
export const loginUser = async (userCredentials) => {
  const { data } = await authApi.post(APP_CONSTANTS.API_ENDPOINTS.AUTH_ENDPOINTS.LOGIN_ENDPOINT, {
    user: userCredentials,
  });
  return data;
};
